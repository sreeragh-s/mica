...
import type { AppMode, NotesUser } from '@/components/notes/notes-app-types'
...
export function useNotesGitSourceControl({
  ...
  const [gitSourceControlActionError, setGitSourceControlActionError] = useState<string | null>(null)
  ...
  const refreshGitSourceControl = useCallback(async (): Promise<GitSourceControlSnapshot | null> => {
    const api = getApi()
    ...
    try {
      ...
      if (snapshot.isRebasing || snapshot.hasConflicts) {
        log.info('refreshGitSourceControl snapshot', {
          ...
          conflictedFiles: snapshot.files.filter((file) => file.conflicted).map((file) => file.path),
        })
      }
      return snapshot
    } finally {
  ...
  }, [primaryGitFolder?.localGitPath, refreshGitSourceControl, refreshWorkspaceGitStatuses, revealConflictResolver, user])
