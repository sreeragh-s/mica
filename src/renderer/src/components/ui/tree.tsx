...
export type TreeProviderProps = {
  ...
  animateExpand?: boolean
  className?: string
  /** Increment to merge `expandNodeIds` into the expanded set (e.g. reveal a folder after creating a child). */
  expandNonce?: number
...
export const TreeProvider = ({
  ...
  useEffect(() => {
    if (expandNonce <= 0 || expandNonce === lastExpandNonce.current) {
      return
    }
  ...
  const handleSelection = useCallback(
    (nodeId: string, ctrlKey = false) => {
      ...
      if (multiSelect && ctrlKey) {
        ...
        // Single-select: always select the clicked node (no toggle-off on second click).
        newSelection = [nodeId]
      }
...
export const TreeNodeTrigger = ({
  ...
  return (
    <motion.div
      className={cn(
        'group relative mx-1 flex cursor-pointer items-center rounded-md px-2 py-1 transition-all duration-200',
        'hover:bg-accent/50',
...
export const TreeExpander = ({
  ...
  if (!hasChildren) {
    return <div className="mr-1 h-3.5 w-3.5 shrink-0" />
  }
  ...
  return (
    <motion.div
      ...
      className={cn(
        'mr-1 flex h-3.5 w-3.5 shrink-0 cursor-pointer items-center justify-center',
        className
      )}
      onClick={(e) => {
        e.stopPropagation()
...
export const TreeIcon = ({ icon, hasChildren = false, className, ...props }: TreeIconProps) => {
  ...
  return (
    <motion.div
      className={cn(
        'mr-1.5 flex h-3.5 w-3.5 shrink-0 items-center justify-center text-muted-foreground',
        className
      )}
