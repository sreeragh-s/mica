 ...
 */
export function markdownToSerializedState(markdown: string): SerializedEditorState | null {
  ...
  try {
    ...
    editor.update(
      () => {
        const root = $getRoot()
        root.clear()
        $convertFromMarkdownString(trimmed, MARKDOWN_TRANSFORMERS, undefined, true)
      },
      { discrete: true }
    )
...
export function diskBodyToContent(body: string, fileTitle: string): SerializedEditorState | null {
