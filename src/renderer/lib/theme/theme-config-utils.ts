...
function sanitizePartial(raw: Record<string, unknown> | undefined): Partial<ThemeStyleProps> {
...
export function sanitizeThemeConfig(raw: unknown): NotelabThemeConfigV1 | undefined {
...
export function isNonEmptyThemeConfig(c: NotelabThemeConfigV1 | undefined | null): boolean {
  if (!c) return false
  return Object.keys(c.light).length > 0 || Object.keys(c.dark).length > 0
}

export function countThemeConfigKeys(c: NotelabThemeConfigV1 | undefined | null): {
  light: number
  dark: number
} {
...
export function buildThemeConfigFromPresetId(presetId: string): NotelabThemeConfigV1 {
  if (presetId === DEFAULT_THEME_PRESET_ID || presetId === CUSTOM_THEME_PRESET_ID) {
