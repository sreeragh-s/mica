...
import { extractPreviewText, formatNoteTime, type SavedNote } from '@/lib/notes/notes-storage'
...
export function UrlFavicon({ href }: { href: string }): JSX.Element {
  ...
  if (href.startsWith('mailto:')) {
    return <Mail className="text-muted-foreground size-4 shrink-0" aria-hidden />
  }
...
export function InternalNoteLinkPreviewBody({
  ...
  const folderLabel =
    resolvedNote && notelabCtx
      ? (notelabCtx.folders.find((f) => f.folder === resolvedNote.folder)?.name ?? 'Workspace')
      : ''
  ...
  return (
    <div className="min-w-0 flex-1 space-y-1">
      <div className="truncate text-sm leading-tight font-medium">
        {resolvedNote ? resolvedNote.title?.trim() || 'Untitled' : 'Missing note'}
      </div>
      ...
      {previewLine ? (
        <p className="text-muted-foreground line-clamp-2 text-xs leading-snug">{previewLine}</p>
      ) : null}
    </div>
  )
}
