...
const arm64Files = allFiles.filter((f) => f.startsWith("notelab.io-") && f.endsWith("-arm64-mac.zip"));
...
if (!hasResults) {
	console.log(`No macOS artifacts found${versionFilter ? ` for version ${versionFilter}` : ""} in ${DIST_DIR}`);
	process.exit(0);
}
